...
@section('title', 'Tambah Struktur Organisasi - Halo MANAP')

@section('admin_content')
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Tambah Posisi</h1>
        <div class="text-sm text-gray-500 mt-1 flex items-center gap-2">
            <span class="text-blue-600">Master Data</span>
            <span class="text-gray-400">/</span>
            <a href="{{ route('admin.organization-hierarchies.index') }}" class="text-blue-600 hover:underline">Struktur Organisasi</a>
            <span class="text-gray-400">/</span>
            <span>Tambah</span>
        </div>
    </div>
    <a href="{{ route('admin.organization-hierarchies.index', ['unit_id' => request('unit_id')]) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        <div class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
            <i class="fa-solid fa-diagram-project text-blue-600 text-sm"></i>
        </div>
        <h2 class="font-semibold text-gray-800">Informasi Posisi Baru</h2>
    </div>
    <div class="p-6">
        @if($errors->any())
        ...
        <form action="{{ route('admin.organization-hierarchies.store') }}" method="POST">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="parent_jabatan_id" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-white">
                            ...
                            @foreach($jabatans as $j)
                                <option value="{{ $j->id }}" {{ old('parent_jabatan_id') == $j->id ? 'selected' : '' }}>
                                    [L{{ $j->level }}] {{ $j->nama }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mb-8 pb-8 border-b border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="workflow_level" value="{{ old('workflow_level', 1) }}" min="1" required
                ...
                <div class="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_workflow_start" value="1" {{ old('is_workflow_start') ? 'checked' : '' }}
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_workflow_end" value="1" {{ old('is_workflow_end') ? 'checked' : '' }}
                            class="w-4 h-4 accent-blue-600 rounded">
                        <div>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="can_escalate" value="1" {{ old('can_escalate', true) ? 'checked' : '' }}
                            class="w-4 h-4 accent-blue-600 rounded">
                        <div>
            ...
            <div class="flex items-center gap-3 pt-6 border-t border-gray-100">
                ...
                <a href="{{ route('admin.organization-hierarchies.index', ['unit_id' => request('unit_id')]) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">
                    Batal
                </a>
            </div>
        </form>
